...
# -- ORIGINAL V2 GMT MAP (always runs - Page 1 of the bulletin) ----------------
# pscoast draws the coastline and land fill for the map region.
/usr/local/bin/gmt pscoast -R$x1/$x2/$y1/$y2  -JM10  -K  -O -Bg0.3 -BWSne -Ba0.3 -Df -P -GNavajoWhite -Wthick  -X1 -Y2.5 >> $ps

# Loop through each probability polygon file (proval-*.xy) and draw coloured hulls
for (( i=0;i<${#ProXY[@]};i++ ));do
        G=${Col[$(echo ${ProXY[$i]} | sed 's/-/ /g' | awk '{print $2}')]}
        /usr/local/bin/gmt psxy  ${ProXY[$i]} -R -J -W0.2 $G -B -O -K  >>$ps
done

# Draw the Last Known Position dot marker on the map
/usr/local/bin/gmt pstext -J -O -K -R  -GWhite -N >>$ps<<END
${inputLon} ${inputLat} 18 0 1 MC .
END

# Draw the uncertainty radius circle around the LKP
...
# --- GENERATE NEW PAGES FOR TIME-BASED PROBABILITY IMAGES ---
pdf_list="firstpage_$1.pdf secondpage_$1.pdf thirdpage_$1.pdf"

page_idx=1
for img in figure/seeding_duration_${1}_combined_page_*.png; do
    if [ -f "$img" ]; then
        ps_page="timepage_${1}_${page_idx}.ps"
        pdf_page="timepage_${1}_${page_idx}.pdf"
        
        # Setup page
        /usr/local/bin/gmt gmtset BASEMAP_TYPE=plain PAPER_MEDIA custom_595x842 PAGE_ORIENTATION landscape
        /usr/local/bin/gmt psbasemap -R76/98/13/28 -JM7.5i -G -V -P -K -X1.5 -Y2.5 -B+t"" > $ps_page
        
        # Place the image
        /usr/local/bin/gmt psimage "$img" -Dx0/0+w10i -O -K >> $ps_page
        
        # Header text
        echo "87 39.3 19 0 5 MC SARAT" | /usr/local/bin/gmt pstext -JM7.5i -N -O -K -R -G -F+f0/0/255 >> $ps_page
        echo "87 38.8 14 0 5 MC Search And Rescue Aid Tool" | /usr/local/bin/gmt pstext -JM7.5i -N -O -K -R -G >> $ps_page
        echo "87 38.4 10 0 5 MC ESSO-INDIAN NATIONAL CENTRE FOR OCEAN INFORMATION SERVICES" | /usr/local/bin/gmt pstext -JM7.5i -N -O -K -R -G >> $ps_page
        echo "87 37.5 10 0 5 MC ISO 9001 : 2008 Certified  Ocean State Forecast Services" | /usr/local/bin/gmt pstext -JM7.5i -N -O -K -R -G >> $ps_page
        echo "87 38 10 0 5 MC (Ministry of Earth Sciences, Government of India)" | /usr/local/bin/gmt pstext -JM7.5i -N -O -K -R -G >> $ps_page
        
        # LKP Text
        echo "77 35.6 11 0 5 BL Last Known Position is Longitude: ${lodeg} deg ${lomin} mins ${losec} secs (${inputLon} deg), Latitude: ${ltdeg} deg ${ltmin} mins ${ltsec} secs (${inputLat} deg)" | /usr/local/bin/gmt pstext -JM7.5i -N -O -K -R -F+f11p,Helvetica,40/40/40 >> $ps_page
        
        # Footer
        echo "88 10.5  In case of any query, please contact 040-23895017 or email us at osf@incois.gov.in" | /usr/local/bin/gmt pstext -JM7.5i -N -O -R -F+f12,-=0.5p,black >> $ps_page

        ps2pdf $ps_page
        pdf_list="$pdf_list $pdf_page"
        page_idx=$((page_idx + 1))
    fi
done
