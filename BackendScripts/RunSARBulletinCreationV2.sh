...
echo "77 33.6 11 0 5 BL Last Known Position is Longitude: ${lodeg} deg ${lomin} mins ${losec} secs, Latitude: ${ltdeg} deg ${ltmin} mins ${ltsec} secs" | /usr/local/bin/gmt pstext -JM7.5i -N -O -K -R -Ff40/40/40 >>$ps
...
echo "77 35.6 11 0 5 BL Last Known Position is Longitude: ${lodeg} deg ${lomin} mins ${losec} secs, Latitude: ${ltdeg} deg ${ltmin} mins ${ltsec} secs" | /usr/local/bin/gmt pstext -JM7.5i -N -O -K -R -Ff40/40/40 >>$ps
...
lomin=$(expr "$lodmin * 60" | bc -l | awk '{print $1}');
        ...
        lomin=$(expr "$lodmin * 60" | bc -l |  awk '{print $1}' | sed 's/\./ /g' | awk '{print $1}' );
losec=$(expr "$lodsec * 60" | bc -l |  awk '{print $1}' | sed 's/\./ /g' | awk '{print $1}');
echo " $lodeg deg $lomin min $losec sec , $ltdeg deg $ltmin min $ltsec sec" >>ddmmsec.txt
